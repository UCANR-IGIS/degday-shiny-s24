...
## For other Bootswatch themes, see https://bootswatch.com/
my_bs_theme <- bs_theme(version = 5, bootswatch = "minty")
...
## Create a 'named vector' of the stations (for display in the selectInput)
stations_named_vec <- setNames(stations_tbl$stid, stations_tbl$name)
...
server <- function(input, output) {
  ...
  output$out_report <- renderUI(
    div(
      p("The first flight is predicted to occur on ",
        strong(format(flight_dates()[1], '%B %d')), "."),
      p("The second flight is predicted to occur on ",
        strong(format(flight_dates()[2], '%B %d')), "."),
